We are cleaning up mypy errors on this active PR. To see the strict mypy errors you MUST use the `just lint` command in the api directory; it runs both baseline checks on all files AND strict checks on all src files in parallel.
...
1. Run `just lint` in api directory to see all strict mypy errors
...
6. After each file: run `just ruff` then `just lint` to verify
