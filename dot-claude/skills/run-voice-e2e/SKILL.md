...
**🚨 COMMON MISTAKE - VITE DEFAULT PORT**: Vite defaults to port 5173. You MUST explicitly configure the port in `frontend/vite.config.ts`:

```typescript
server: {
  port: 5174,  // MUST match ct4 directory!
  // ... other config
}
```

**If you see Vite running on 5173, it's WRONG for ct4.** Kill it and restart after fixing the config.
...
```typescript
server: {
  port: 5174,  // Must be 5174 for ct4!
  // ...
}
```
...
**If you see port 5173, STOP and fix vite.config.ts first!**
